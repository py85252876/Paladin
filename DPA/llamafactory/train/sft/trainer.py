...
class CustomSeq2SeqTrainer(Seq2SeqTrainer):
    ...
    def add_tag(self,generated_ids) -> torch.Tensor:
        ...
        for t in high_entropy_positions:
            ...
            with torch.no_grad():
                ...
                temperature = 1.5  # 调整温度
                delta = 0.7  # 扰动强度
                target_token = self.tokenizer.encode("email", add_special_tokens=False)[0]
        ...
        if not self.remake_tokens:
            ...
            for inputs in inputs_list:
                ...
                for i in range(len(inputs['input_ids'])):
                    ...
                    input_query = self.tokenizer.batch_decode(query, skip_special_tokens=True, clean_up_tokenization_spaces=False)[0]
                    # pattern = r"### Instruction:\n([\s\S]*?)\n\n### Response"
                    # print(self.tokenizer.batch_decode(inputs_id))
                    # print(re.search(pattern, input_query).group(1))
                    # match = re.search(pattern, input_query)
                    # input_match = self.tokenizer(match.group(1), truncation=True, padding=True, max_length=512, return_tensors="pt")
    ...
    def evaluate_model(self, model: nn.Module, epoch) -> torch.Tensor:
        ...
        with torch.no_grad():
            eval_outputs = model.generate(input_ids=input_tokens.input_ids, 
                                        max_length=512, 
                                        use_cache=True,
                                        do_sample=True,
                                        top_p = 1.0,
            ...
            if self.ref_model is not None:
                ref_outputs = self.ref_model.generate(input_ids=input_tokens.input_ids, 
                                                    max_length=512, 
                                                    use_cache=True,
                                                    do_sample=True,
                                                    top_p = 1.0,
